...
(function (factory) {
  ...
  (function ($, moment) {
    ...
    function Calendar(element, instanceOptions) {
      ...
      function renderView(delta, viewName) {
        ...
        const originalDate = new Date(date._d);
        var start_date=currentView.start._d.getFullYear() + '-' + 
        ('0' + (currentView.start._d.getMonth() + 1)).slice(-2) + '-' + 
        ('0' + currentView.start._d.getDate()).slice(-2)
        var end_date=currentView.end._d.getFullYear() + '-' + 
        ('0' + (currentView.end._d.getMonth() + 1)).slice(-2) + '-' + 
        ('0' + currentView.end._d.getDate()).slice(-2)


        var new_dateList = getDateList(start_date,end_date);
        var c = replaceDateListIfNotExists(new_dateList, already_existdatelist);

          // var check=['2024-03-01','2024-03-06','2024-03-02'].includes('2024-03-09')




        const ajaxyear = originalDate.getFullYear();
        const ajaxmonth = ('0' + (originalDate.getMonth() + 1)).slice(-2); // Adding 1 to month as it is zero-based
        const ajaxday = ('0' + originalDate.getDate()).slice(-2);
        
        const formattedDate = `${ajaxyear}-${ajaxmonth}-${ajaxday}`;
        if (c == false){
          already_existdatelist=new_dateList
          getevents(ajaxday,ajaxmonth,ajaxyear,already_existdatelist)   

        }
        if (ifmonthchanged != ajaxmonth){
          ifmonthchanged=ajaxmonth
         // getevents(ajaxday,ajaxmonth,ajaxyear,already_existdatelist)   
      ...
      function replaceDateListIfNotExists(new_datelist, already_exist_datelist) {
        var allExist = true;
       

        for (var i = 0; i < new_datelist.length; i++) {
  
            if (!already_exist_datelist.includes(new_datelist[i])) {
                allExist = false;
                break;
            }
        }
    ...
    function Header(calendar, options) {
      ...
      function render() {
        ...
        if (sections) {
          ...
          .append($('<div class="fc-right" id="rightsection"/>').append($('<div/>').append(renderSection("right"))).append($('<div/>').append(renderSection("left"))))
  ...
  */
  ...
  $(function () {
    ...
    const ajaxyear = todayDate.format("YYYY");
    const ajaxmonth = todayDate.format("MM");
    const ajaxday = todayDate.format("DD");
