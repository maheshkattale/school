...
class timetablelist(GenericAPIView):
    ...
    def post(self,request):
        ...
        dateobjs = TimeTable.objects.filter(startdate=startdate,enddate=enddate,isActive=True,school_code=schoolcode)
        dateser = TimeTableSerializer(dateobjs,many=True)
        for d in dateser.data:
            teacherid = d['TeacherId']
            teacherobj = User.objects.filter(id=teacherid).first()
            if teacherobj is not None:
                d['teachername'] = teacherobj.Username
            else:
                d['teachername'] = "--"
            ...
            if subjectobj is not None:
                d['subject'] = subjectobj.SubjectName
            else:
                d['subject'] = "--"
        print("dateser.data",dateser.data)
        return Response({"data":dateser,"response": {"n": 1, "msg": "teachers found Successfully","status": "Success"}})
